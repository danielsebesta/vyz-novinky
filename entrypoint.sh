...
echo "0 12 * * * root cd /app && /usr/local/bin/python daily_quiz.py >> /var/log/quiz.log 2>&1" > /etc/cron.d/quiz-cron
...
echo "[$(date)] Container started. Cron scheduled for 12:00 UTC daily."
...
if [ "${RUN_ON_START:-false}" = "true" ]; then
    ...
    cd /app && python daily_quiz.py 2>&1 | tee -a /var/log/quiz.log
fi
